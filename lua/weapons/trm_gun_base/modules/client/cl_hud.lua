...
function DebugHUD(ply, wep)
    ...
    draw.SimpleText("Current Task: " .. (wep.GetCurrentTask and wep:GetCurrentTask() or "None"), "Default", ScrW() / 2, ScrH() * 0.74, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local aimdelta = wep:GetAimDelta()
    draw.SimpleText("Aim Delta: " .. (aimdelta and math.Round(aimdelta, 2) or "None"), "Default", ScrW() / 2, ScrH() * 0.76, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local sequence = wep.m_CurrentSequence or wep:GetPlayingSequence() or "None"
    draw.SimpleText("Sequence: " .. (sequence and sequence or "None"), "Default", ScrW() / 2, ScrH() * 0.78, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local text = wep:CanPrimaryAttack() and "1" or "0"
    draw.SimpleText("Can: " .. (text and text or "None"), "Default", ScrW() / 2, ScrH() * 0.68, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local text = wep:GetOwner():GetViewModel(0):GetCycle()
    text = math.Round(text, 2)
    draw.SimpleText("Cycle: " .. (text and text or "None"), "Default", ScrW() / 2, ScrH() * 0.65, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local text = CurTime() - wep:GetNextPrimaryFire() >= 0 and "true" or "false"
    draw.SimpleText("Next: " .. (text and text or "None"), "Default", ScrW() / 2, ScrH() * 0.25, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local text = ply:GetViewModel(0):SequenceDuration()
    text = math.Round(text, 2)
    draw.SimpleText("SeqDur: " .. (text and text or "None"), "Default", ScrW() / 2, ScrH() * 0.22, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local vRecoil = wep.m_VRecoil or Angle(0, 0, 0)
    local back = wep:GetVisualRecoilBackward()
    draw.SimpleText(string.format("Visual Recoil: P=%.2f Y=%.2f", vRecoil.pitch, vRecoil.yaw), "Default", ScrW() / 2, ScrH() * 0.80, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    draw.SimpleText(string.format("Visual Recoil Backward: %.2f", back), "Default", ScrW() / 2, ScrH() * 0.82, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end
